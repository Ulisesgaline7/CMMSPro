<x-layouts.cmms :title="'Editar: ' . $plan->name" :headerTitle="'Editar: ' . $plan->name">
...
<div class="p-8 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h2 class="text-2xl font-extrabold text-[#002046] font-headline tracking-tight">Editar: {{ $plan->name }}</h2>
            <p class="text-sm text-gray-400 mt-0.5">Modifica los campos del plan de mantenimiento</p>
        </div>
    </div>

    <form method="POST" action="{{ route('maintenance-plans.update', $plan) }}" class="space-y-6">
        @csrf
        @method('PATCH')

        @include('maintenance-plans.partials.form', ['plan' => $plan])
