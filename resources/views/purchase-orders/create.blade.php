<x-layouts.cmms title="Nueva Orden de Compra" headerTitle="Nueva Orden de Compra">
...
<div class="p-8 max-w-5xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h2 class="text-2xl font-extrabold text-[#002046] font-headline tracking-tight">Nueva Orden de Compra</h2>
    ...
    <form method="POST" action="{{ route('purchase-orders.store') }}" class="space-y-6"
          x-data="{
              ...
              items: {{ old('items') ? json_encode(array_map(fn($i) => ['description' => $i['description'] ?? '', 'part_number' => $i['part_number'] ?? '', 'quantity' => $i['quantity'] ?? 1, 'unit' => $i['unit'] ?? 'pz', 'unit_price' => $i['unit_price'] ?? 0], old('items'))) : '[{description: \'\', part_number: \'\', quantity: 1, unit: \'pz\', unit_price: 0}]' }},
              get total() {
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="supplier_name" value="{{ old('supplier_name') }}"
                           ...
                           placeholder="Ej. Ferretería Industrial S.A.">
                    @error('supplier_name')
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-4">
            ...
            <div>
                ...
                <textarea name="notes" rows="3"
                          class="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#002046]/20 focus:border-[#002046] resize-none"
                          placeholder="Instrucciones especiales, condiciones de entrega...">{{ old('notes') }}</textarea>
            </div>
        </div>
