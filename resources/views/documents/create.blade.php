<x-layouts.cmms title="Nuevo Documento" headerTitle="Nuevo Documento">
...
<div class="p-8 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h2 class="text-2xl font-extrabold text-[#002046] font-headline tracking-tight">Nuevo Documento</h2>
            <p class="text-sm text-gray-400 mt-0.5">Completa los campos para registrar el documento</p>
        </div>
    </div>

    <form method="POST" action="{{ route('documents.store') }}" class="space-y-6"
          x-data="{ type: '{{ old('type', 'procedure') }}' }">
        @csrf

        @include('documents.partials.form', ['document' => null, 'assets' => $assets])
