<x-layouts.cmms :title="'Editar – ' . $location->name" headerTitle="Editar Ubicación">
...
<div class="p-8 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            ...
            @if ($location->code)
                <p class="text-sm text-gray-400 mt-0.5 font-mono">{{ $location->code }}</p>
            @endif
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('locations.update', $location) }}" class="space-y-6">
        ...
        @include('locations.partials.form', ['location' => $location, 'parents' => $parents])
