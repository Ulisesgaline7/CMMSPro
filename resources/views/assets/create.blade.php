<x-layouts.cmms title="Nuevo Activo" headerTitle="Nuevo Activo">
...
<div class="p-8 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h2 class="text-2xl font-extrabold text-[#002046] font-headline tracking-tight">Nuevo Activo</h2>
            <p class="text-sm text-gray-400 mt-0.5">Completa los campos para registrar el activo</p>
        </div>
    </div>

    <form method="POST" action="{{ route('assets.store') }}" class="space-y-6"
          x-data="{ status: '{{ old('status', 'active') }}', criticality: '{{ old('criticality', 'medium') }}' }">
        @csrf
