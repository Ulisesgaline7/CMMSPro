<x-layouts.cmms :title="'Editar – ' . $part->name" headerTitle="Editar Repuesto">
...
<div class="p-8 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            ...
            @if ($part->part_number)
                <p class="text-sm text-gray-400 mt-0.5 font-mono">{{ $part->part_number }}</p>
            @endif
        </div>
    </div>

    <form method="POST" action="{{ route('inventory.update', $part) }}" class="space-y-6">
        @csrf
        @method('PATCH')

        @include('inventory.partials.form', ['part' => $part])
