<x-layouts.cmms title="Órdenes de Trabajo" headerTitle="Órdenes de Trabajo">
    ...
    @php
        ...
        $priorityColors = ['low' => 'text-gray-400', 'medium' => 'text-blue-500', 'high' => 'text-orange-500', 'critical' => 'text-red-600'];
    ...
    <div class="p-6 space-y-5">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
            @php
                $statCards = [
                    ...
                    ['label' => 'Completadas Hoy', 'value' => $stats['completed_today'], 'color' => 'text-green-600'],
        ...
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-extrabold text-[#002046] font-headline tracking-tight">Órdenes de Trabajo</h2>
                <p class="text-sm text-gray-400 mt-0.5">{{ $workOrders->total() }} {{ $workOrders->total() === 1 ? 'orden registrada' : 'órdenes registradas' }}</p>
            </div>
        ...
        <form method="GET" action="{{ route('work-orders.index') }}"
            ...
            <select name="type" onchange="this.form.submit()"
                ...
                <option value="corrective" {{ ($filters['type'] ?? '') === 'corrective' ? 'selected' : '' }}>Correctivo (CM)</option>
                <option value="preventive" {{ ($filters['type'] ?? '') === 'preventive' ? 'selected' : '' }}>Preventivo (PM)</option>
                <option value="predictive" {{ ($filters['type'] ?? '') === 'predictive' ? 'selected' : '' }}>Predictivo (PdM)</option>
            </select>
        ...
        <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            @if ($workOrders->isEmpty())
                <div class="flex flex-col items-center justify-center py-16 text-center">
                    <i data-lucide="clipboard-list" class="w-12 h-12 text-gray-200 mb-3"></i>
                    <p class="text-gray-500 font-medium">No se encontraron órdenes</p>
                    <p class="text-gray-400 text-sm mt-1">
                        {{ array_filter($filters) ? 'Intenta con otros filtros' : 'Crea la primera orden de trabajo' }}
                    </p>
                </div>
            @else
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100 bg-gray-50/60">
                            <th class="text-left px-5 py-3 text-xs font-bold uppercase tracking-wider text-gray-500">Orden</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($workOrders as $wo)
                            @php
                                ...
                                $isOverdue   = in_array($statusVal, ['pending', 'in_progress']) && $wo->due_date && $wo->due_date->isPast();
                            @endphp
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-5 py-3.5">
                                    <div class="flex items-center gap-2">
                                        <span class="text-[10px] font-bold px-1.5 py-0.5 rounded {{ $typeColors[$typeVal] ?? 'bg-gray-100 text-gray-600' }}">
                                            {{ $typeAbbrev[$typeVal] ?? $typeVal }}
                                        </span>
                                        <div>
                                            <div class="font-semibold text-[#002046]">{{ $wo->title }}</div>
                                            <div class="text-xs text-gray-400 font-mono mt-0.5">{{ $wo->code }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3.5 hidden md:table-cell text-gray-600">
                                    {{ optional($wo->asset)->name ?? '—' }}
                                </td>
                                <td class="px-4 py-3.5 hidden lg:table-cell text-gray-600">
                                    {{ optional($wo->assignedTo)->name ?? '—' }}
                                </td>
        ...
        @if ($workOrders->hasPages())
            <div class="flex items-center justify-between text-sm text-gray-500">
                <span>{{ $workOrders->firstItem() }}–{{ $workOrders->lastItem() }} de {{ $workOrders->total() }}</span>
                {{ $workOrders->withQueryString()->links('pagination::simple-tailwind') }}
            </div>
        @endif
