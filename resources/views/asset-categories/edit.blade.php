<x-layouts.cmms :title="'Editar – ' . $category->name" headerTitle="Editar Categoría">
...
<div class="p-8 max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            ...
            @if ($category->code)
                <p class="text-sm text-gray-400 mt-0.5 font-mono">{{ $category->code }}</p>
            @endif
        </div>
    </div>

    <form method="POST" action="{{ route('asset-categories.update', $category) }}" class="space-y-6">
        @csrf
        @method('PATCH')

        @include('asset-categories.partials.form', ['category' => $category])
