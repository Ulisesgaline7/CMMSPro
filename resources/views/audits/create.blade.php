<x-layouts.cmms title="Nueva Auditoría" headerTitle="Nueva Auditoría">
...
<div class="p-8 max-w-3xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h2 class="text-2xl font-extrabold text-[#002046] font-headline tracking-tight">Nueva Auditoría</h2>
            <p class="text-sm text-gray-400 mt-0.5">Completa los campos para planificar la auditoría</p>
        </div>
    </div>

    <form method="POST" action="{{ route('audits.store') }}" class="space-y-6"
          x-data="{ type: '{{ old('type', 'internal') }}' }">
        @csrf

        @include('audits.partials.form', ['audit' => null])
